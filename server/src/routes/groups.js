...
// List groups in a program
router.get('/programs/:programId/groups', requireAuth, async (req, res) => {
...
router.post('/programs/:programId/groups', requireAuth, requireProgramRoleMin('HEAD_COACH'), async (req, res) => {
  ...
  const group = await prisma.group.create({
    data: {
      ...
      groupNumber,
      name,
      coaches: coachUserIds
        ? { create: coachUserIds.map((uid) => ({ userId: uid })) }
        : undefined,
    },
...
router.patch('/programs/:programId/groups/:groupId', requireAuth, requireProgramRoleMin('HEAD_COACH'), async (req, res) => {
...
router.delete('/programs/:programId/groups/:groupId', requireAuth, requireProgramRoleMin('HEAD_COACH'), async (req, res) => {
  const prisma = getPrisma();
